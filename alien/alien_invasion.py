...
class AlienInvasion:
    ...
    def __init__(self):
        ...
        # 加载游戏准备界面的背景图像
        self.background_ready = pygame.image.load(self.settings.ready_background_image).convert()
        self.background_ready = pygame.transform.scale(self.background_ready,
                                                       (self.settings.screen_width, self.settings.screen_height))

        # 加载背景图像
        self.background = pygame.image.load(self.settings.game_background_image).convert()
    ...
    def _update_aliens(self):
        ...
        if current_time - self.last_alien_spawn_time > self.settings.alien_spawn_time:
            self._create_alien()
